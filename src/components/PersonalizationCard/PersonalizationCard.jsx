...
function LoginPopup({ onClose, onLogin }) {
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      {/* Modal Card */}
      <div className="relative bg-white rounded-3xl shadow-2xl px-8 py-8 mx-4 max-w-sm w-full flex flex-col items-center gap-4 animate-[popIn_0.25s_ease-out]" onClick={(e) => e.stopPropagation()}>
        {/* Tombol Close */}
        ...
        <div className="text-center font-[Poppins]">
          ...
          <p className="text-sm text-gray-500 mt-1 leading-relaxed">Kamu perlu login untuk menyimpan dan melihat hasil personalisasi kamu.</p>
        </div>
...
export default function PersonalizationCard({ smart, styleRes, readOnly = false }) {
  ...
  const undertoneImage = undertone === "WARM" ? warmImg : undertone === "COOL" ? coolImg : neutralImg;
  ...
  async function handleSave() {
    ...
    try {
      ...
      await api.post("/personalization", {
        userId,
  ...
  function handleLogin() {
    sessionStorage.setItem("redirectAfterLogin", "/hasil-personalisasi");
    ...
    <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl p-10">
      ...
      <div className="w-full mb-3 flex flex-col items-center gap-6">
        {displayImages.map((imgSrc, idx) => (
          <img key={idx} src={imgSrc} alt={`Style ${idx + 1}`} className="w-full max-w-[550px] max-h-[550px] object-contain rounded-2xl" />
        ))}
      </div>
      ...
      {resultSentence && <p className="text-center text-gray-700 mt-1 mb-6 max-w-xl mx-auto font-[Poppins]">{resultSentence}</p>}
      ...
      {/* Warna */}
      <section className="mb-8 font-[Poppins]">
        <h3 className="font-semibold text-pink1 text-xl mb-3">Rekomendasi Warna ({undertone})</h3>
      ...
      {!readOnly && (
        <div className="mt-10 flex flex-col gap-3">
          <button onClick={handleSave} disabled={saving} className="w-full py-3 rounded-full bg-pink1 hover:bg-oren2 text-white font-medium text-lg transition-all disabled:opacity-70">
            {saving ? "Saving..." : "Save Result"}
          </button>
          <button onClick={handleClose} className="w-full py-3 rounded-full bg-gray-400 hover:bg-gray-300 text-white font-medium text-lg transition-all">
            Close
          </button>
        </div>
      )}
