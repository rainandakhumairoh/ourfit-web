    ...
    <div className="relative w-full overflow-hidden pb-32">
      ...
      <div className="relative w-full bg-pink2 pb-16">
        ...
        <div className="relative w-full flex flex-col items-center justify-center bg-cover bg-center">
          ...
          <img src={bgBordir} alt="Bordir bawah" className="absolute bottom-[-14em] w-full h-auto object-cover z-0" />
          ...
          <div className="absolute top-[50%] left-1/2 -translate-x-1/2 w-[85%] max-w-5xl flex justify-center gap-6 px-6 z-20">
            {mixmatch.slice(0, 3).map((item) => (
              <div key={item._id} className="w-[300px] h-[300px] bg-white rounded-xl shadow-md flex items-center justify-center border-2 border-oren1">
                {item.image ? (
      ...
      <div className="relative z-30 flex justify-center mt-0">
        <Link to="/mixmatch" className="border border-white text-white bg-pink1 rounded-full px-16 py-2 text-sm flex items-center gap-2 hover:bg-white hover:text-[#B13B3B] transition-all duration-300 shadow-md">
          Lihat Selengkapnya
        </Link>
      </div>
    </div>
