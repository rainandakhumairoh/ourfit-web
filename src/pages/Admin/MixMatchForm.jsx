...
export default function MixMatchForm({ refresh }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/mixmatch", formData, {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {selectedProducts.length === 0 && (
          <p className="text-xs text-gray-400 mt-2">
            Belum ada produk dipilih. Minimal 1 produk.
          </p>
        )}
      </div>
      ...
      <button
        type="submit"
        className="bg-pink1 hover:bg-oren2 text-white py-3 rounded-full font-semibold transition mt-2"
      >
        Tambah Mix & Match
      </button>
    </form>
