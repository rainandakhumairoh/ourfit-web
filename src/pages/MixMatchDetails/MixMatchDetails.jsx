  ...
  const handleBookmarkToggle = async () => {
    ...
    try {
      if (isSaved) {
        // Hapus bookmark
        await axios.delete(
          `http://localhost:5000/api/bookmarks/${id}?userId=${currentUser.id}`
        );
        setIsSaved(false);
      } else {
        // Tambah bookmark
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col bg-primary pt-8">
        ...
        <div className="text-center pt-12">
          <h2 className="text-center text-coklat font-bold text-2xl uppercase mb-2">
            {item.title}
          </h2>
          {item.category && (
            <span className="inline-block px-3 py-1 bg-[#f4cda3] text-[#5a2e0f] text-xs font-semibold rounded-full">
              {item.category}
            </span>
          )}
        </div>
