...
export default function Favorite() {
  ...
  const handleRemove = async (productId, favoriteId) => {
    const userId = currentUser.id || currentUser._id;
    try {
      await api.delete(`/favorite/${productId}?userId=${userId}`);
  ...
  return (
    <div className="min-h-screen font-[Poppins] bg-coklat p-10 pt-28">
      <div className="max-w-6xl mx-auto">
        <button onClick={() => navigate(-1)} className="fixed top-24 left-8 bg-pink2 text-white p-3 rounded-full shadow-md hover:bg-oren2 transition z-50">
          <ArrowLeft size={20} />
        </button>

        <h2 className="text-center text-white font-bold text-3xl uppercase mb-8">Favorit Saya</h2>
        ...
        {!loading && favorite.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-white/70 text-sm mt-1">Klik ikon ❤ di halaman produk untuk menyimpan!</p>
            <button onClick={() => navigate("/wardrobe")} className="mt-6 px-6 py-2 bg-pink2 text-white rounded-full text-sm font-medium hover:bg-oren2 transition-colors">
              Jelajahi Produk
            </button>
          </div>
        )}
        ...
        {!loading && favorite.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {favorite.map((item) => (
              <FavoriteCard key={item._id} item={item} onRemove={handleRemove} />
            ))}
          </div>
        )}
      </div>
