...
export default function MixMatch() {
  ...
  const filtered = activeCategory === "All" ? mixes : mixes.filter((m) => m.category?.toLowerCase() === activeCategory.toLowerCase());

  return (
    <>
      <MixMatchTopSection activeCategory={activeCategory} onCategoryChange={setActiveCategory} currentUser={currentUser} />
      <div className="min-h-screen bg-pink2 p-10 pt-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map((mix) => (
              <MixMatchCard key={mix.id} item={mix} onSaveToggle={handleSaveToggle} isSaved={saved.includes(mix.id)} />
            ))}
          </div>
        </div>
      </div>
